...
func TestQueryScheduledWorkouts_DedupesStaleGhostCoachPlanEntries(t *testing.T) {
	...
	// item — must collapse to exactly 1, and must not leak a stale nonzero
	// scheduled_id (get_scheduled_workouts documents scheduled_id == 0 for
	// coach-plan items) — routedMockClient with no plan rows isolates the
	// scheduled_workout dedupe path from mergeTrainingPlanDetail entirely.
	...
	if workouts[0].ScheduledID != 0 {
		t.Errorf("ScheduledID: got %d, want 0 — must not leak a stale ghost's churned id", workouts[0].ScheduledID)
	}
}

func TestQueryScheduledWorkouts_DedupePrefersCorrectEntryRegardlessOfOrder(t *testing.T) {
	// Same as above but with the correctly-tagged (scheduled_id == 0) entry FIRST
	// in row order, to prove selection isn't just "keep whichever comes first."
	dateStr := time.Now().UTC().Add(24 * time.Hour).Format(time.RFC3339)
	client := routedMockClient(
		[]map[string]any{
			{"time": dateStr, "name": "Threshold", "sport": "running", "workout_name": "Threshold"},
	...
	if workouts[0].ScheduledID != 0 {
		t.Errorf("ScheduledID: got %d, want 0", workouts[0].ScheduledID)
	}
}
