...
const serverInstructions = `Waypoint exposes read-only Garmin Connect fitness data (synced to InfluxDB) plus one write tool for queuing structured workouts.

Data domains and their tools:
- Activities: get_recent_activities (list), get_weekly_volume (aggregated by sport/week). Use an activity's id from get_recent_activities with get_activity_splits (per-lap) and get_activity_hr_zones (time in HR zone) for detail on one activity.
- Daily health: get_daily_stats (steps, resting HR, body battery, stress), get_sleep_summary, get_hrv_trend, get_respiration.
- Training status: get_training_status (Garmin's own overreaching/peaking status + VO2max), get_training_readiness (day-to-day readiness score, informed by HRV/sleep — see get_hrv_trend/get_sleep_summary for the underlying detail), get_training_load (computed ATL/CTL/TSB from activity data, not a Garmin field).
