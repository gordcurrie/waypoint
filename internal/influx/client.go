...
func (c *Client) queryJSON(ctx context.Context, sql string, params map[string]any) ([]map[string]any, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		if readErr != nil {
			return nil, fmt.Errorf("influx.Query: status %d (body unreadable: %v)", resp.StatusCode, readErr)
		}
...
func (c *Client) writeLineProtocol(ctx context.Context, lp string) error {
	...
	if resp.StatusCode >= 300 {
		...
		if readErr != nil {
			return fmt.Errorf("influx.Write: status %d (body unreadable: %v)", resp.StatusCode, readErr)
		}
