...
func TestPointLineProtocol(t *testing.T) {
	p := influx.NewPoint("training_load").
...
func TestPointLineProtocol_NoTags(t *testing.T) {
	p := influx.NewPoint("training_load").SetField("atl_7day", 42.3).SetTimestamp(lpTimestamp)
	lp := p.LineProtocol()
...
func TestPointLineProtocol_SpecialCharsEscaped(t *testing.T) {
	p := influx.NewPoint("my measurement").
...
func TestPointLineProtocol_EmptyFields(t *testing.T) {
	p := influx.NewPoint("training_load").SetTimestamp(time.Now())
...
func TestPointLineProtocol_EmptyMeasurement(t *testing.T) {
	p := influx.NewPoint("").SetField("x", 1.0).SetTimestamp(time.Now())
	if got := p.LineProtocol(); got != "" {
...
func TestPointLineProtocol_NaNFieldDropped(t *testing.T) {
	p := influx.NewPoint("m").SetField("valid", 1.0).SetField("nan_val", math.NaN()).SetTimestamp(lpTimestamp)
	lp := p.LineProtocol()
...
func TestPointLineProtocol_InfFieldDropped(t *testing.T) {
	p := influx.NewPoint("m").SetField("inf_val", math.Inf(1)).SetField("ok", 2.0).SetTimestamp(lpTimestamp)
	lp := p.LineProtocol()
...
func TestPointSetTimestamp_ZeroIgnored(t *testing.T) {
	before := time.Now()
	p := influx.NewPoint("m").SetField("f", 1.0).SetTimestamp(time.Time{})
	lp := p.LineProtocol()
	after := time.Now()
	var ns int64
