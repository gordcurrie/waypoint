...
// TrainingReadiness represents one row from the "training_readiness" measurement.
// HRVStatus is numerically encoded: 2.0=BALANCED, 1.0=UNBALANCED, 0.0=POOR.
// HRVStatus is a pointer so nil (absent) can be distinguished from 0.0 (POOR).
type TrainingReadiness struct {
	...
	ACWPct        *float64 `json:"acw_pct,omitempty"`
}
...
func TrainingReadinessFrom(row map[string]any) TrainingReadiness {
	return TrainingReadiness{
		...
		ACWPct:        roundFPtr(floatPtrFrom(row, "acw_pct")),
	}
}
