...
- **MCP server**: Go — exposes 16 fitness data tools to Claude (or any MCP client)
...
**Available tools:** `get_recent_activities`, `get_weekly_volume`, `get_daily_stats`, `get_sleep_summary`, `get_hrv_trend`, `get_respiration`, `get_training_status`, `get_training_load`, `get_training_readiness`, `get_performance_trend`, `get_lactate_threshold`, `get_activity_splits`, `get_activity_hr_zones`, `get_scheduled_workouts`, `create_workout`, `search_exercises`

Each tool's description documents how it relates to the others (e.g. which tool to call first, which tools cover the same data at a different granularity) — the server also sends this as connection-level `Instructions` (see `cmd/mcp-server/main.go`) so most MCP clients surface it without an extra call.
...
| `DRIFT_ALERT_WEBHOOK_URL` | — | Optional. POSTed `{method, kind, date, errors}` on a schema mismatch or a new field Garmin added; unset = log-only |
...
`sync/drift_check.py` sends a plain `POST` with a JSON body — `{"method": "...", "kind":
"mismatch"|"new_fields", "date": "YYYY-MM-DD", "errors": ["..."]}`, `Content-Type:
application/json`, no auth headers, no retries. `kind` distinguishes something actually
broken (`"mismatch"`) from Garmin having added a field nothing syncs yet (`"new_fields"`,
non-urgent — additive, not breaking); the two alert and dedupe independently, so getting
one doesn't suppress the other for the same method on the same day. This shape is generic
enough for **any** endpoint that accepts a raw JSON POST (n8n, a serverless function, Home
Assistant's webhook trigger, your own tiny HTTP server, etc.) — it is *not* pre-shaped for
Slack's or Discord's incoming-webhook formats, which expect their own specific JSON
(`{"text": ...}`, embeds, etc.); routing to either of those needs a translation step in
between, same as the n8n path below.
...
2. Add a **Set** (or **Function**) node to turn the payload into a message, e.g.
   `{{$json.kind === "new_fields" ? "New field(s)" : "Drift"}} on {{$json.method}}
   ({{$json.date}}): {{$json.errors.join(", ")}}`.
...
6. Test end-to-end without waiting for a real drift: `curl -X POST -H 'Content-Type:
   application/json' -d '{"method":"test","kind":"mismatch","date":"2026-01-01","errors":
   ["smoke test"]}' "$DRIFT_ALERT_WEBHOOK_URL"` — confirm the Telegram message arrives.
